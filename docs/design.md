...
MVP では固定トークンによる簡易認証を実装する。ただし以下の制約を明示する。

> **⚠️ この認証方式はローカル環境での個人利用を前提とした最低限の保護です。**
> 外部ネットワークへの公開は絶対に行わないでください。
> XSS 等への脆弱性があるため、本番運用には適しません。

| 項目 | 仕様 |
|------|------|
| トークン管理 | `.env` の `APP_TOKEN` で設定 |
| フロント送信 | 全リクエストの `Authorization: Bearer {token}` ヘッダーに付与 |
| トークン保存 | localStorage（XSS・Chrome 拡張・同一 PC 内の他プロセスからは保護されない。ローカル限定のため許容。） |
